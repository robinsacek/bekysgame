...
**Status: the agreed V4 M1-M6 implementation is locally complete and validated: 113 simulation tests, all eight full browser scenarios, all six Chromium/WebKit living-world cases, desktop/mobile visual checks, integrated-browser controls, and identical standalone/Pages builds. The V4-only work remains on `checkpoint/v4-2026-09-09`; concurrent feeding integration and the combined release are separate outstanding work. Do not push an unfinished build to `main`, which deploys Pages automatically. Moving tides, additional species, and engine replacement remain excluded.**
...
- **Final gates:** the complete V4 local gates now pass on the final artifact identified in the authoritative handoff below. Earlier partial results and restart notes are historical. Feeding integration, combined-build verification, and hosted release remain outstanding.
...
## Checkpoint and Release Handoff (2026-09-09)

**Authoritative current handoff; supersedes the paused and earlier authorization/next-action notes above.** The user authorized checkpoint commit/push and final V4 validation, followed by reconciliation and release after the concurrent session finishes. V4 local acceptance is complete; that does not certify the separately owned feeding build or its eventual hosted release.

### Checkpoint State

- **Branch/worktree:** `checkpoint/v4-2026-09-09` in `C:\Git\Blob-Island-v4-checkpoint`, based on `f8ad8a6f8df9cae31a12bfae94df229a07e0d097`. The branch is the V4-only checkpoint; use its exact fetched commit when integrating, not a loose working-directory copy.
- **Isolation complete:** feeding-only lifecycle, harvesting, pose, bubble-snapshot and browser hooks were removed only from this isolated copy. V4 patch feeding, trait-aware meal timing, gaze/comedy, current/wakes, held-input guards, adaptive rendering, and the retained raft-exit recovery are preserved. The shared `C:\Git\Blob-Island` checkout, index, branch and concurrent feeding source were not rewritten or reset by this extraction.
- **Final artifact:** the standalone and local Pages builds are **9,068,314 bytes**, SHA-256 `9e479b6ef5ccfbbdea12f7a0b826022029fd3783728fed55b48495933b420cd4`. A fresh `--pages` rebuild exactly reproduced the tested bytes in both outputs. No deployment occurred. The earlier `df4189...` checkpoint artifact and its failures/results remain historical evidence, not proof of these new bytes.
- **Simulation and browser acceptance:** **113/113** tests pass, including picnic target geometry and the eight-second raft-exit control. All **eight full responsive/input/audio/objective scenarios** pass on the final artifact, as the union of `v4-final-portrait-resolution-2-browser-results.json` (one portrait case) and `v4-final-seven-resolution-browser-results.json` (the other seven). Counts, identities, and hashes were checked; these are full workflows, not audio-only or visual-only substitutes.
- **Living-world acceptance:** `v4-final-six-living-attempt-1.json` records **all six** Chromium/WebKit map cases passing on the final hash, each with at least 435,000 ms of actual simulated activity. Real manipulation, recovery, natural meals/comedy, pursuit, bounded state, and zero untouched objective progress passed. Windows WebKit's audio controls are honestly disabled; Chromium covers working voices and music.
- **Visual and integrated acceptance:** `v4-final-visual-attempt-1-browser-results.json` covers all-map desktop/phone shore, reef, signature events, and production pose galleries. `v4-final-rendering.json` records 4,954 changed refraction pixels inside the silhouette and zero outside, 4,243,207 cache pixels, 8,372 scratch pixels, and the new adaptive-resolution control. `v4-final-integrated-browser.json` records actual editor-browser mouse drag/release, pause/resume, independent music/effects, all-map nonblank pixels, and journal navigation at 1513 by 1250. Screenshots were inspected. These ignored receipts live in the isolated worktree's `test-results/`; rerun the applicable suites if they are unavailable.
- **Boundary:** V4's agreed local implementation and validation are complete. Feeding integration and the combined hosted release are not. Hardware performance/listening, strict soft-body self-intersection prevention, and the previously excluded candidates are not newly claimed as complete.

### Final Validation Repairs

- **Picnic routing:** a valid coconut delivery could select a blocked-side snack target outside the objective's shared-visit radius and stop there indefinitely. Candidate targets now stay within the clearing, with the existing obstacle checks, arrival tolerance, objective radius, dwell, and player-input requirements unchanged. The new six-fixture target regression and all objective positives/negatives pass.
- **Severe rendering slowdown:** after refraction and caustics are disabled, sustained slow rendering can reduce only the backing canvas to 75 percent of CSS resolution. Severe frame averages above 50 ms use a 20-frame response instead of the normal 45-frame response. World dimensions, picking coordinates, cached layers, and active grips remain unchanged; the rendering regression proves the minimum dimensions, one retained grip, identical world snapshot, and 239 sampled colors. The previously failing full portrait scenario passes its original deadline. This is bounded degradation, not a 30/60 fps hardware claim.
- **Input fixture:** the coast carry helper compensates for an off-center material grip and places Blobby on clear lookout ground instead of dropping onto the crate. Delivery/settlement snapshots are retained. Objective assertions, negative controls, and completion deadlines were not weakened. Initial tablet-picnic, phone-landing, and portrait-frame failures remain in uniquely named attempt receipts.

### Next Owner and Integration

1. **Concurrent feeding-session owner:** finish and preserve the [issue #1](https://github.com/robinsacek/bekysgame/issues/1) implementation in the shared checkout, including its tests and art. Commit/push that work on a non-deploying branch when stable, without discarding the V4 edits it already contains. Do not copy the checkpoint's V4-only files over the shared combined source. Report the exact feeding commit, tests, and rebuilt HTML hash.
2. **Reconciliation owner (next coding session):** fetch both branch tips and inspect their actual merge base. Use a clean integration worktree. The feeding branch may already contain V4 source because it began in the shared dirty checkout; compare the branch trees and intended delta before applying changes so V4 is not duplicated or undone. Preserve both branches before resolving overlap; no forced resets, blanket `ours`/`theirs`, or force pushes.
3. **Resolve the shared files deliberately:** [src/foraging.js](src/foraging.js) and [src/wildlife.js](src/wildlife.js) must keep V4 energy/traits, ordinary decision timing, held/threat/recovery/objective priorities, the retained raft-exit route, and clearing-bounded picnic targets alongside feeding reservations and consumption. Retain the new target regression in [src/objectives.test.js](src/objectives.test.js). [src/physics.js](src/physics.js) and [src/game.js](src/game.js) must preserve independent grips, current/wake limits and inactive timing when consumed food releases its own pointer. Avoid per-frame random decision draws introduced solely by active feeding.
4. **Resolve presentation/testing overlap:** combine gaze/comic poses with feeding faces in [src/creature-pose.js](src/creature-pose.js), [src/creature-art.js](src/creature-art.js), and [src/character-details.js](src/character-details.js); retain wind, water ordering, refraction/quality budgets, adaptive backing resolution, and food drawing in [src/forage-art.js](src/forage-art.js) and [src/scene.js](src/scene.js). Carry over the feeding session's bubble files/tests and browser helper as a unit. Preserve the WebKit canvas flush, co-temporal palm snapshot, and adaptive-resolution regression in [tools/browser-test.cjs](tools/browser-test.cjs), plus grip-offset delivery and its observations in [tools/coast-browser.cjs](tools/coast-browser.cjs). Do not merge [index.html](index.html) by choosing one side: rebuild it from the reconciled source and retained license notices.

### Release Gates and Stop Conditions

- **Success criteria:** one identified combined artifact passes all simulation tests (113 V4 tests plus the feeding coverage), the standalone build, all eight full responsive/input/audio/objective scenarios, all six Chromium/WebKit living-world cases, feeding positives/negative controls, and desktop/mobile visual and integrated-browser inspection. The snapshot must work without runtime errors. Both effects and music remain default-off and independently suspended; named residents and all four objectives remain intact.
- Run `npm test`, `npm run build`, `npm run test:browser`, and `npm run test:living` from the actual integration-worktree directory, with temporary `BLOB_FEATURES`, `BLOB_CASES`, and living-suite selectors cleared for full gates. Inspect counts and receipt hashes; do not accept a green selector run as the full matrix. Preserve failed evidence and rerun affected gates after source/build changes.
- **Publication owner:** only after both workstreams are stable, conflicts are reconciled, the gates pass, and the validated commits are pushed, merge the intended release into `main`. [The Pages workflow](.github/workflows/pages.yml) deploys every push to `main`; the checkpoint branch does not trigger it. Check remote `main` for intervening commits before pushing and never force it.
- Build with `--pages` and prove standalone/Pages byte identity, verify the deployment's source commit and hosted HTML hash/HTTP status, then exercise hosted input/audio/objectives and report browser-engine limits honestly. Windows WebKit's disabled-audio fallback is not physical-iPad testing.
- **Stop/coordinate** if either session is still writing the files to be integrated, the fetched commits differ from the handoff, the snapshot/build fails, or test/source hashes do not match. Do not publish an in-flight mixture or relabel old receipts. The shortest authorized path is: preserve both commits, reconcile in isolation, rebuild once, complete the required gates on those bytes, then release.
