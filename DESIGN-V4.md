...
**Status: the V4-only checkpoint is separated and verified with 112 passing simulation tests and an exact rebuild of the Chromium-verified artifact. Checkpoint commit/push is authorized on `checkpoint/v4-2026-09-09`; combined feeding integration and release gates remain outstanding. Do not push an unfinished build to `main`, which deploys Pages automatically. Moving tides, additional species, and engine replacement remain excluded.**
...
**Authoritative current handoff; supersedes the paused and earlier authorization/next-action notes above.** The user authorized a checkpoint commit/push, followed by reconciliation and release after the concurrent session finishes. This is not a claim that the complete V4 release gates have passed.
...
- **Fresh verification:** `npm test` in the isolated worktree passed **112/112**, including the eight-second raft-exit control. The standalone build is **9,067,912 bytes** with SHA-256 `df4189b752e6a3c9916445609fdb9c58f3f5d5a37b965de671b34691e52c1141`, exactly matching the pre-feeding V4 artifact. No unresolved editor diagnostics in the extracted files. This equality permits reuse of that artifact's browser observations, not observations from later feeding builds.
- **Existing browser evidence:** all three Chromium living cases passed on these exact bytes: Lagoon 436,383 ms, Pools 435,050 ms, Sunset 435,050 ms. Full receipts are in the shared checkout's ignored `test-results/v4-living-chromium-lagoon.json` and `test-results/v4-living-chromium-pools-sunset.json`. M2-M6 visual receipts and repertoire galleries remain in the shared `test-results/` directory. These local evidence files are not source files in the pushed branch; rerun the applicable suites when they are unavailable.
- **Not release-ready:** the complete eight-scenario responsive/input/audio/objective matrix and three WebKit living cases still need a final passing receipt. Older partial runs are not a combined-build PASS. Hardware performance/listening, strict soft-body self-intersection prevention, and the previously excluded candidates are not newly claimed as complete.
...
3. **Resolve the shared files deliberately:** [src/foraging.js](src/foraging.js) and [src/wildlife.js](src/wildlife.js) must keep V4 energy/traits, ordinary decision timing, held/threat/recovery/objective priorities, and the retained raft-exit route alongside feeding reservations and consumption. [src/physics.js](src/physics.js) and [src/game.js](src/game.js) must preserve independent grips, current/wake limits and inactive timing when consumed food releases its own pointer. Avoid per-frame random decision draws introduced solely by active feeding.
4. **Resolve presentation/testing overlap:** combine gaze/comic poses with feeding faces in [src/creature-pose.js](src/creature-pose.js), [src/creature-art.js](src/creature-art.js), and [src/character-details.js](src/character-details.js); retain wind, water ordering, refraction/quality budgets, and food drawing in [src/forage-art.js](src/forage-art.js) and [src/scene.js](src/scene.js). Carry over the feeding session's bubble files/tests and browser helper as a unit. Preserve the WebKit canvas flush and co-temporal palm snapshot in [tools/browser-test.cjs](tools/browser-test.cjs). Do not merge [index.html](index.html) by choosing one side: rebuild it from the reconciled source and retained license notices.

### Release Gates and Stop Conditions

- **Success criteria:** one identified combined artifact passes all simulation tests (112 V4 tests plus the feeding coverage), the standalone build, all eight full responsive/input/audio/objective scenarios, all six Chromium/WebKit living-world cases, feeding positives/negative controls, and desktop/mobile visual and integrated-browser inspection. The snapshot must work without runtime errors. Both effects and music remain default-off and independently suspended; named residents and all four objectives remain intact.
