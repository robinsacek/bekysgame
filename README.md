...
The prioritized scope and verification record are in [DESIGN-V4.md](DESIGN-V4.md). Moving tides, additional species, engine replacement, and experimental soft-mesh self-collision remain deferred. The live canvas is capped at two million pixels; sustained slow rendering disables refraction/caustics and can lower backing resolution to 75 percent of CSS pixels without changing world coordinates or active grips. Scenery caches stay below 4.5 million pixels and refraction scratch below 512 by 512. Desktop/tablet frame-rate targets still require representative physical-device measurements; browser emulation is not a hardware certification.

The finalized V4-only build passes 113 simulation tests, all eight full responsive/input/audio/objective browser scenarios, all six Chromium/WebKit living-world cases, and desktop/mobile visual plus integrated-browser checks. Standalone and local Pages builds have identical bytes. Concurrent draggable feeding integration and its combined release remain separate; this V4 build has not been published to Pages.
