...
- Puddle and Sprig catch the same flies as the lizards with flicking tongues. Their ordinary hops have a crouch, springy legs, and a soft landing; occasional playful jumps include a tucked somersault. Drop them into the ocean to swim comfortably at the surface with paddling legs. Flowering lily pads bob with the waves and support frogs on every island. Swimming frogs seek free pads, jump onto them, sit for a while, hop to a nearby leaf, and jump back into the water. Held frogs, meals, and recovery take priority over acrobatics.
...
For just ocean swimming and lily pads, use `$env:BLOB_FEATURES='swimming'` with the same browser cases. Real input carries and releases frogs into every ocean and onto pads; checks cover sustained swimming, paddling strokes, physical support, visible seated frogs, regrabbing, natural hops onto/between/off pads, pause/resume, and map retention. The normal frog and full browser suites include these checks too.
