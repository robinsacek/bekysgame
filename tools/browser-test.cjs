    ...
    try {
      for (const config of configurations.filter(item => item.engine === engineName)) {
        ...
        try {
          ...
          if (!['audio', 'visual'].includes(features)) await exerciseCoast(page, config, output);
          ...
          const result = { name: config.name, engine: engineName, passed: true, colors: initial.colors, viewport: config.viewport, input: config.nativeTouch ? 'native multi-touch' : config.touch ? 'WebKit touch pointers and native tap' : 'mouse', networkMode: browserOffline ? 'browser-offline' : 'external-requests-blocked', audio: audioResult, visual: visualResult };
