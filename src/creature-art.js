...
export function drawCreature(art, context, resident, time) {
  ...
  if (species === 'fish') {
    ...
    const pulse = feeding.eating ? feeding.open : resident.antic === 'hiccup' && resident.anticUntil > time ? (Math.sin(time * 0.025) + 1) / 2 : resident.pulse || 0;
    ...
    if (!resident.frown && !feeding.eating && !feeding.smile && resident.snapUntil <= time && !(resident.antic === 'yawn' && resident.anticUntil > time)) { context.beginPath(); context.moveTo(34, 6); context.quadraticCurveTo(39, 8, 45, 3); context.stroke(); }
    ...
    if (!resident.frown && !feeding.eating && !feeding.smile) { context.strokeStyle = paint(ink, 0.65); context.lineWidth = 1.1; context.beginPath(); context.arc(0, 2, 3.5, 0.2, 2.9); context.stroke(); }
    ...
    if (!resident.frown && !feeding.eating && !feeding.smile) { context.beginPath(); context.arc(31, 3, 3, 0.1, 1.8); context.stroke(); }
    ...
    const beakOpen = feeding.open * 6;
    shape(context, [[18, -9], [beakLength, -6 - beakOpen * 0.4], [18, -6]], paint(resident.appearance === 'sandpiper' ? wood : yellow));
    shape(context, [[18, -6], [beakLength - 1, -6 + beakOpen], [18, -3 + beakOpen * 0.45]], paint(resident.appearance === 'sandpiper' ? wood : yellow));
    if (resident.appearance === 'pelican') { context.fillStyle = paint(mix(yellow, paper, 0.42)); context.beginPath(); context.moveTo(18, -3); context.quadraticCurveTo(29, 5 + beakOpen, beakLength - 1, -6 + beakOpen); context.closePath(); context.fill(); }
