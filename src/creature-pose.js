...
function applyCreaturePose(context, resident, time) {
  ...
  if (resident.anticUntil > time) {
    if (resident.antic === 'cartwheel') context.rotate(Math.min(Math.PI * 2, (time - resident.anticUntil + 2500) / 1200 * Math.PI * 2));
    if (resident.antic === 'binky') context.rotate(Math.sin((time - resident.anticUntil + 2500) * 0.009) * 0.13);
  }
...
module.exports = { drawingSize, applyCreaturePose };