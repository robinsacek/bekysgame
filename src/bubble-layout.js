function layoutBubbles(residents, time, view) {
  ...
  for (const resident of visible) {
    const kind = resident.ewwUntil > time ? 'ewww' : resident.thoughtUntil > time ? resident.thought : null;
    if (!kind) continue;
    const radiusX = (kind === 'ewww' ? 29 : 15) * scale;
    ...
    const offsets = [[0, -radiusY - 12 * scale], [-radiusX * 2.1, -radiusY - 12 * scale], [radiusX * 2.1, -radiusY - 12 * scale], [0, -radiusY * 3.4], [-radiusX * 2.1, -radiusY * 3.4], [radiusX * 2.1, -radiusY * 3.4], [radiusX * 2.2, headroom + 3 * scale]];
    for (const [offsetX, offsetY] of offsets) {
      ...
      bubbles.push({ ...bounds, x: positionX, y: positionY, radiusX, radiusY, anchorX: resident.x, anchorY, id: resident.id, kind, alpha: Math.min(1, ((kind === 'ewww' ? resident.ewwUntil : resident.thoughtUntil) - time) / 180) });
      break;
    }
  }
  return bubbles;
